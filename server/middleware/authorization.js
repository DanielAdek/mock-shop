...
export const verifyToken = (req, res, next) => {
  try {
    ...
    if (!tokenBearer) return res.status(403).json(errorResponse('Forbiden', 403, 'headers:{Authorization}', 'Authorize user', 'Client key is required. Access Denied!', { error: true, operationStatus: 'Processs Terminated!' }));
    ...
    JWT.verify(token, secret, async (err, decoded) => {
      ...
      const user = await Services.retreiveOneData(User, { id: decoded.id });
